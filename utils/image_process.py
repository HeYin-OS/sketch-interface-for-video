    ...
    for i in range(points.shape[0]):  # for all candidate paths
        for j in range(picking_limit):  # for all candidate points of prev stroke point
            ...
            for k in range(picking_limit):  # for all candidates points of next stroke point
                ...
                if h < 0.0:
                    h = 1.0 + ti.math.tanh(h)
                else:
                    h = 1.0
                weights[i, j, k] = ti.math.distance((p2 - p1), (q2 - q1)) / ti.math.pow(r, 2) + alpha * h
...
def affine_and_trim_ti(q1: ti.types.vector(2, ti.f64),
    ...
    start_point = ti.Vector([m_new_homo[0] - (img_trim.m >> 1), m_new_homo[1] - (img_trim.n >> 1)], dt=ti.i32)
    for ki in range(img_trim.n):
        for kj in range(img_trim.m):
...
def convolution_ti(dog_kernel: ti.types.matrix(n=1, m=3, dtype=ti.f64),
    ...
    for i in range(dog_fld.n):
        for j in range(dog_fld.m):
            temp = ti.f64(0.0)
            for ki in range(dog_fld.n):
                for kj in range(dog_fld.m):
                    temp += img_fld[i + ki, j + kj] * dog_kernel[ki, kj]
            dog_fld[i, j] = temp
    # gaussian filter
    gs_fld = ti.Matrix.zero(ti.f64, 14, 8)
    for i in range(gs_fld.n):
        for j in range(gs_fld.m):
            temp = ti.f64(0.0)
            for ki in range(gs_fld.n):
                for kj in range(gs_fld.m):
...
@ti.kernel
def affine_and_integral_ti_new(points: ti.template(),
                               candidate_points: ti.template(),
                               gray_image: ti.template(),
                               dog_kernel: ti.types.matrix(n=1, m=3, dtype=ti.f64),
                               gaussian_kernel: ti.types.matrix(n=3, m=1, dtype=ti.f64),
